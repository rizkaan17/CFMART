...
namespace CFMART.Controllers
{
    ...
    {
        private ContextProduk _db = new ContextProduk();
        // Ambil semua produk
        public List<Produk> GetAllProduk()
        {
            return _db.GetAllProduk();
        }
        ...
        public List<Produk> SearchProduk(string keyword)
        {
            ...
            var cmd = new NpgsqlCommand(
                @"SELECT id_produk, jenis_produk, harga, stok 
                  FROM produk 
                  WHERE LOWER(jenis_produk) LIKE LOWER(@keyword)", conn);
        ...
        // Tambah produk baru
        public bool TambahProduk(string nama, double harga, int stok)
        {
            Produk p = new Produk
            {
                Jenis_Produk = nama,
            ...
            return _db.AddProduk(p);
        }

        // Edit produk
        public bool EditProduk(int id, string nama, double harga, int stok)
        {
            Produk p = new Produk
            {
                Id_Produk = id,
        ...
        public (bool sukses, string pesan) HapusProduk(int id)
        {
            bool sukses = _db.DeleteProduk(id);
            return sukses ? (true, "Produk berhasil dihapus.") : (false, "Gagal hapus, produk mungkin terkait transaksi.");
        }

        // 6. DETAIL: Ambil detail by ID (Penting untuk Form Edit)
        public Produk GetProdukById(int id)
        {
            return _db.GetProdukById(id);
        }
